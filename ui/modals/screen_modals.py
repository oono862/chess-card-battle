...
def show_settings_screen(screen, W, H, get_font, bgm_manager):
    """Simple settings screen to toggle BGM ON/OFF and adjust volume.
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                mx, my = event.pos
                # 戻るボタン判定
                back_btn_rect = pygame.Rect(x + w - 120, y + h - 60, 100, 40)
                if back_btn_rect.collidepoint(mx, my):
                    return
                
                # BGM ON/OFFトグルボタン
                toggle_rect = pygame.Rect(x + 40, y + 70, 180, 40)
                if toggle_rect.collidepoint(mx, my):
                    current = bgm_manager.get_bgm_enabled()
                    bgm_manager.set_bgm_enabled(not current)
                
                # ボリュームスライダー
                if slider_x <= mx <= slider_x + slider_w and slider_y - 10 <= my <= slider_y + slider_h + 10:
                    dragging = True
                    ratio = (mx - slider_x) / slider_w
                    bgm_manager.set_bgm_volume(max(0.0, min(1.0, ratio)))
        ...
        title_surf = title_font.render("設定", True, (0, 0, 0))
        modal_surf.blit(title_surf, ((w - title_surf.get_width()) // 2, 20))

        # BGM ON/OFF
        label_font = get_font(22)
        bgm_label = label_font.render("BGM:", True, (0, 0, 0))
        modal_surf.blit(bgm_label, (40, 80))
        
        bgm_enabled = bgm_manager.get_bgm_enabled()
        toggle_text = "ON" if bgm_enabled else "OFF"
        toggle_color = (100, 200, 100) if bgm_enabled else (200, 100, 100)
        pygame.draw.rect(modal_surf, toggle_color, (180, 70, 100, 40))
        pygame.draw.rect(modal_surf, (0, 0, 0), (180, 70, 100, 40), 2)
        toggle_surf = label_font.render(toggle_text, True, (255, 255, 255))
        modal_surf.blit(toggle_surf, (180 + (100 - toggle_surf.get_width()) // 2,
                                      70 + (40 - toggle_surf.get_height()) // 2))

        # ボリュームスライダー
        vol_label = label_font.render("音量:", True, (0, 0, 0))
        modal_surf.blit(vol_label, (40, 140))
        
        volume = bgm_manager.get_bgm_volume()
        # スライダー背景
        pygame.draw.rect(modal_surf, (180, 180, 180), (slider_x - x, slider_y - y, slider_w, slider_h))
        # スライダーつまみ
        knob_x = int(slider_x - x + slider_w * volume)
        pygame.draw.circle(modal_surf, (100, 150, 220), (knob_x, slider_y - y + slider_h // 2), 12)
        pygame.draw.circle(modal_surf, (255, 255, 255), (knob_x, slider_y - y + slider_h // 2), 12, 2)
        
        # 音量パーセント表示
        vol_pct = int(volume * 100)
        pct_surf = label_font.render(f"{vol_pct}%", True, (0, 0, 0))
        modal_surf.blit(pct_surf, (slider_x - x + slider_w + 20, slider_y - y - 10))

        # 戻るボタン
        back_btn_rect = pygame.Rect(w - 120, h - 60, 100, 40)
        pygame.draw.rect(modal_surf, (100, 150, 220), back_btn_rect)
        pygame.draw.rect(modal_surf, (255, 255, 255), back_btn_rect, 2)
        back_font = get_font(20)
        back_text = back_font.render("戻る", True, (255, 255, 255))
        modal_surf.blit(back_text, (back_btn_rect.x + (back_btn_rect.w - back_text.get_width()) // 2,
                                    back_btn_rect.y + (back_btn_rect.h - back_text.get_height()) // 2))
